...
class PRTReId(DetectionLevelModule):
    ...
    def __init__(
        ...
        self.model = None
        self.nb_try_calls = 0
        self.nb_except_calls = 0
        print("On insstancie PRTReid")
    ...
    def preprocess(self, image, detection: pd.Series, metadata: pd.Series):
        ...
        try:
            ...
            # Vérification de coordonnées valides
            if any(coord is None or isinstance(coord, float) and np.isnan(coord) for coord in [l, t, r, b]):
                raise ValueError(f"BBox ltrb contient des valeurs invalides : {[l, t, r, b]}")

            crop = image[t:b, l:r]

            # Vérification si crop est vide
            if crop.size == 0:
                raise ValueError("Crop vide détecté")


            if self.nb_except_calls>0:
                print("on a quand même toujours des trucs valide")
            ...
            self.nb_except_calls += 1

            print("\n" + "=" * 80)

            print("❌ [PRTReId] ERREUR dans preprocess()")

            print(f"[EXCEPTION] : {e}")

            print(f"[INFO] Image shape : {image.shape}")

            print(f"[INFO] detection.name : {detection.name}")

            print(f"[DEBUG] bbox_ltwh type = {type(detection.get('bbox_ltwh', None))}")

            print(f"[DEBUG] bbox_ltwh brut = {detection.get('bbox_ltwh', 'ABSENT')}")

            bbox_accessor = detection.bbox

            print(f"[DEBUG] Type de detection.bbox : {type(bbox_accessor)}")

            try:

                print("[DEBUG] Tentative ltrb() manuelle :")

                l, t, r, b = bbox_accessor.ltrb(

                    image_shape=(image.shape[1], image.shape[0]), rounded=True

                )

                print(f"[⚠️] Surprise : ltrb() marche en except : {(l, t, r, b)}")

            except Exception as bbox_error:

                print(f"[❌ ERREUR .ltrb()] : {bbox_error}")

            print("=" * 80 + "\n")

            fallback_crop = np.zeros((133, 50, 3), dtype=np.uint8)
            quit()

            return {"img": Unbatchable([fallback_crop])}
