    ...
    def __init__(self, checkpoint, image_width, image_height, batch_size, device, **kwargs):
        ...
        if Path(checkpoint).name == "train_59.pt":
            ...
            download_file("https://tib.eu/cloud/s/x68XnTcZmsY4Jpg/download/train_59.pt",
                          checkpoint, md5)
        checkpoint = torch.load(checkpoint, map_location=torch.device("cpu"), weights_only=False)  # Charge sur CPU
        self.model.load_state_dict(checkpoint["model"], strict=False)  # Charge les poids
        self.model.to(torch.device("mps"))  # Envoie le modèle sur GPU MPS
        self.model.eval()
...
class TVCalib(ImageLevelModule):
    ...
    def process(self, batch: Any, detections: pd.DataFrame, metadatas: pd.DataFrame):
        ...
        print("\n🧠 [TVCalib] === DÉMARRAGE optimisation ===")

        # Check si le batch est vide
        print(f"📦 Batch size: {batch['lines__ndc_projected_selection_shuffled'].shape[0]}")

        # Check device
        device = next(self.model.parameters()).device
        print(f"💻 Device utilisé pour self_optim_batch: {device}")

        # Vérifie MPS / CUDA / CPU
        print(f"⚙️ torch.backends.mps.is_available(): {torch.backends.mps.is_available()}")
        print(f"⚙️ torch.cuda.is_available(): {torch.cuda.is_available()}")

        # Nombre de cores dispo
        print(f"🧩 Nombre de cores logiques: {psutil.cpu_count(logical=True)}")
        print(f"🧩 Nombre de cores physiques: {psutil.cpu_count(logical=False)}")

        # Charge CPU actuelle
        print(f"🧠 Charge CPU actuelle: {psutil.cpu_percent(interval=0.5)} %")

        # Mémoire dispo
        mem = psutil.virtual_memory()
        print(
            f"💾 Mémoire libre : {round(mem.available / 1024 ** 3, 2)} Go / {round(mem.total / 1024 ** 3, 2)} Go totaux")

        # Timestamp de début
        start = time.time()
        print(f"⏱️ Début à {start:.2f}")
        ...
        print(f"✅ self_optim_batch terminé en {time.time() - start:.2f} s\n")
