...
def compute_vaep_from_predictions(spadl_h5, predictions_h5):
    A = []
    complex_features_parquet = "data-fifa/features_complex2_fulll.parquet"
    df_complex = pd.read_parquet(complex_features_parquet)
    valid_game_ids = set(df_complex["game_id"].unique())
...
def train_vaep_models_and_predict(X, Y, games, spadl_h5, predictions_h5):
    ...
    # Étape 2 : Encodage one-hot (sans la première modalité pour éviter la colinéarité)
    ...
    for col in Y.columns:
        ...
        model.fit(X, y)
        print(f"🔍 SHAP analysis for target: {col}")
        X_numeric = X.copy()
        X_numeric = X_numeric.apply(pd.to_numeric, errors="coerce")  # convertit tout en numérique
        X_numeric = X_numeric.fillna(0.0).astype(np.float64)  # remplace les NaN, cast propre

        explainer = shap.Explainer(model, X_numeric, feature_perturbation="interventional")
        shap_values = explainer(X_numeric)

        # 1. Summary Plot
        shap.summary_plot(shap_values, X_numeric, show=False)  # show=False si tu es en script
        plt.title(f"SHAP Summary Plot for '{col}'")
        plt.tight_layout()
        plt.savefig(f"outputs_complex/shap_summary_{col}.png")
        plt.close()

        # 2. (Optionnel) Feature Importance bar plot
        shap.plots.bar(shap_values, show=False)
        plt.title(f"SHAP Bar Plot for '{col}'")
        plt.tight_layout()
        plt.savefig(f"outputs_complex/shap_bar_{col}.png")
        plt.close()
        models[col] = model
...
def load_vaep_data(spadl_h5, features_h5, labels_h5, complex_features_parquet, nb_prev_actions=1, use_complex_features=True):
    import socceraction.vaep.features as fs
    import pandas as pd
    import tqdm

    # 1. Charger les game_id valides depuis ton .parquet
    df_complex = pd.read_parquet(complex_features_parquet)
    valid_game_ids = set(df_complex["game_id"].unique())

    # 2. Préparer les complex features si besoin
    if use_complex_features:

        complex_features = df_complex[[
            "game_id", "action_id",
            "pressure_ratio", "passing_opportunities",
            "n_teammates_ahead", "n_opponents_ahead",
            "confidence_opponents", "confidence_total"
        ]]
    else:
        complex_features = None

    # 3. Charger les jeux de données uniquement pour les matchs valides
    games = pd.read_hdf(spadl_h5, "games")
    games = games[games.game_id.isin(valid_game_ids)].reset_index(drop=True)
    ...
    for game_id in tqdm.tqdm(games.game_id, desc="Loading features & labels"):
        try:
            ...
            assert len(Xi) == len(actions), f"Mismatch: game_id {game_id} has {len(Xi)} features but {len(actions)} actions"
            Xi["action_id"] = actions["action_id"].values
            Xi["game_id"] = game_id
            Yi = pd.read_hdf(labels_h5, f"game_{game_id}")[["scores", "concedes"]]

            if len(Xi) == len(Yi):
                if use_complex_features and complex_features is not None:
                    Xi = Xi.merge(complex_features, on=["game_id", "action_id"], how="left")
                X.append(Xi)
                Y.append(Yi)
...
if __name__ == '__main__':
    ...
    predictions_h5 = os.path.join(datafolder, "predictions_short_dataset_with_complex_features.h5")

    #print("🔁 Chargement des données...")
    complex_features_parquet = "data-fifa/features_complex2_fulll.parquet"
    """
    games, X, Y = load_vaep_data(
        spadl_h5="data-fifa/spadl-statsbomb3.h5",
        features_h5="data-fifa/features3.h5",
        labels_h5="data-fifa/labels3.h5",
        complex_features_parquet=complex_features_parquet,
