...
class TDEEDModel(BaseRGBModel):
    ...
    class Impl(nn.Module):
        ...
        def forward(self, x, y = None, inference=False, augment_inference=False):
            ...
            #print(f"[forward] x initial shape: {x.shape}")
            ...
            if not inference:
                ...
                #print(f"[forward] Inference mode: normalize done → {x.shape}")
                x = x.view(-1, channels, height, width)
                #print(f"[forward] After flatten to (B*T, C, H, W): {x.shape}")
                if self.croping != None:
                    #print(f"[forward] Applying center crop to {self.croping}x{self.croping}")
                    height = self.croping
                    width = self.croping
                x = self.cropI(x) #same center crop for all frames
                #print(f"[forward] After cropI: {x.shape}")
                x = x.view(batch_size, true_clip_len, channels, height, width)
        ...
        def update_pred_head(self, num_classes = [1, 1]):
            ...
            self._pred_fine = self._pred_fine.to(torch.device("mps" if torch.backends.mps.is_available() else "cpu"))
    ...
    def predict(self, seq, use_amp=True, augment_inference = False):
        ...
        with torch.no_grad():
            if self.device.type == "mps":
                use_amp = False  # ❌ autocast not supported on MPS
            with torch.autocast(device_type=self.device.type) if use_amp else nullcontext():
